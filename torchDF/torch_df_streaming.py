...
class ExportableStreamingTorchDF(nn.Module):
    def __init__(self, fft_size, hop_size, nb_bands,
                 enc, df_dec, erb_dec, erb_indices=None, df_order=5, lookahead=2,
        ...
        # Determine conv_ch. In DF3 it's not explicitly stored in Encoder.
        df_conv0_ch = 16
        for m in self.enc.df_conv0:
            if isinstance(m, nn.Conv2d):
                df_conv0_ch = m.out_channels
                break
        self.rolling_c0_buf_shape = (1, df_conv0_ch, self.df_order, self.nb_df) # [B, conv hidden, df_order, nb_df]
    ...
    def forward_enc(self, feat_erb: Tensor, feat_spec: Tensor, h: Tensor):
        ...
        emb, h = self.enc.emb_gru(emb, h)
        lsnr = self.enc.lsnr_fc(emb) * self.enc.lsnr_scale + self.enc.lsnr_offset
        return e0, e1, e2, e3, emb, c0, lsnr, h

    def forward_erb_dec(self, emb: Tensor, e3: Tensor, e2: Tensor, e1: Tensor, e0: Tensor, h: Tensor):
        b, _, t, f8 = e3.shape
        emb, h = self.erb_dec.emb_gru(emb, h)
    ...
    def forward_df_dec(self, emb: Tensor, c0: Tensor, h: Tensor):
        b, t, _ = emb.shape
        c, h = self.df_dec.df_gru(emb, h)
    ...
    def forward(self, 
        ...
        e0, e1, e2, e3, emb, c0, lsnr, new_enc_hidden = self.forward_enc(
            new_rolling_erb_buf, 
        ...
        # erb_dec
        # [BS=1, 1, T=1, ERB]
        new_gains, new_erb_dec_hidden = self.forward_erb_dec(emb, e3, e2, e1, e0, erb_dec_hidden)
        ...
        new_coefs, new_df_dec_hidden = self.forward_df_dec(
            emb, 
            new_rolling_c0_buf, 
            df_dec_hidden
        )
...
class TorchDFPipeline(nn.Module):
    def __init__(
        ...
        model, state, _, _ = init_df(config_allow_defaults=True, model_base_dir=model_base_dir)
        model.eval()
        self.sample_rate = state.sr()
        self.nb_bands = state.nb_erb()
        self.nb_df = model.nb_df

        self.torch_streaming_model = ExportableStreamingTorchDF(
            nb_bands=self.nb_bands, hop_size=state.hop_size(), fft_size=state.fft_size(), 
            enc=model.enc, df_dec=model.df_dec, erb_dec=model.erb_dec, 
            erb_indices=state.erb_widths(),
            df_order=model.df_order,
            always_apply_all_stages=always_apply_all_stages,
            conv_lookahead=conv_lookahead, nb_df=self.nb_df, sr=self.sample_rate
        )
